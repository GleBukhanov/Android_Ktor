package com.example.android5.network
...
interface KtorNetworkApi {
    suspend fun getCharacters(): List<ApiResponse>
    suspend fun getCharacterById(id: Int): ApiResponse?
}
...
class KtorNetwork : KtorNetworkApi {
    ...
    override suspend fun getCharacters(): List<ApiResponse> {
        return try {
            client.get {
                url {
    ...
    override suspend fun getCharacterById(id: Int): ApiResponse? {
        return try {
            client.get {
                url {
                    host = NETWORK_BASE_URL
